...
function showw() {
  ...
  var DEFAULTaltothisdiv = ''
  var anchothisdiv = DEFAULTanchothisdiv
  var altothisdiv = DEFAULTaltothisdiv
  ...
  for (a = 0; a < spaces.length; a++) {
    ...
    if (spaces[a].length > 1) {
      for (var i = 1; i < spaces[a].length; i++) {
        ...
        */
        ...
        if (spaces[a][i].substring(0, 1) == '_' && spaces[a][i].length > 3 && spaces[a][i].indexOf('.') > 0) {
    ...
    if (spaces[a][0] != null) {
      ...
      if (ht == 'http' || ht == '-htt' || ht == 'ihtt' || ht == 'ahtt') {
        ...
        if (cadjoin.substring(0, 1) == 'h') {
          var spch = cadjoin.indexOf(' ')
          var spch1 = cadjoin.substring(0, spcw)
          //var spcw2=mytrim(spcw1)
          altothisdiv = spch1.substring(1)
          //var spcw3=parseInt(spcw1.substring(1))
          //if(spcw3<=100){
          //anchodiv = spcw3 + '%'
          //}
          //console.log('w:'+anchodiv)
          cadjoin = cadjoin.substring(spch + 1)
        }
        ...
        if (hcontain == -1) {
          ...
          if (prethisdiv) {
            ...
            var atd=''
            if(altothisdiv!=''){atd='height:'+altothisdiv+'; display: inline-block;overflow-y: hidden;'}
            prethis = tag[0] + '<div style="' + tag[2] + 'padding:' + epad + 'px;border-radius:' + radioParagraph + 'px; margin: 0 auto; margin-top:' + emarginTop + 'px;' + imgg + 'background-color:' + oc[0] + '; color:' + oc[1] + ';width:' + anchothisdiv +';'+ atd +'opacity:' + opac + ';overflow-wrap: break-word;">'
            //console.log(atd)
          }
          if (posthisdiv) {
            posthis = '</div>' + tag[1] + '';
          ...
          if (hcontain == 0) {
            ...
            var atd=''
            if(altothisdiv != ''){
              atd ='height:'+altothisdiv+'; display: inline-block; overflow-y: hidden;'
             // console.log(atd)
            }

              prethis = tag[0] + '<div style="' + tag[2] + 'border-radius:' + radioParagraph + 'px;padding:' + epadding + 'px;margin:0 auto;margin-top:' + emarginTop + 'px;' + imgg + 'background-color:' + oc[0] + '; color:' + oc[1] + ';width:' + anchothisdiv+';' +atd+ 'opacity:' + opac + ';overflow-wrap: break-word;">'
            }
            if (posthisdiv) {
              posthis = '</div>' + tag[1] + '';
            }
        ...
        altothisdiv = DEFAULTaltothisdiv
...
$(document).ready(function() {
  ...
  var mody = '<div id="myModal" class="modal"><div class="modal-content"><span class="close">×</span><div id="textomodal"></div></div></div>'
  $('body').append(mody);
